...
reference = (function()
	local main = create({
		{1,"Frame",{BackgroundColor3=Color3.new(0.14117647707462,0.14117647707462,0.14509804546833),BackgroundTransparency=1,BorderColor3=Color3.new(0.15686275064945,0.15686275064945,0.15686275064945),BorderSizePixel=0,Name="Reference",Position=UDim2.new(0.5,-250,0,-500),Size=UDim2.new(0,500,0,20),ZIndex=10,}},
		...
		{104,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Header",Parent={103},Position=UDim2.new(0,8,0,5),Size=UDim2.new(1,-8,0,20),Text="Get Further Help",TextColor3=Color3.new(1,1,1),TextSize=20,TextXAlignment=0,ZIndex=10,}},
...
IYMouse.KeyDown:connect(function(Key)
	...
	elseif infJump == true and Key == " " then
		game.Workspace.CloneCharacter.Humanoid:ChangeState(3)
	end
end)
...
Players.PlayerRemoving:connect(function(player)
	...
	if viewing ~= nil and player == viewing then
		workspace.CurrentCamera.CameraSubject = game.Workspace.CloneCharacter
		viewing = nil
		if viewDied then
			viewDied:Disconnect()
...
CMDs[#CMDs + 1] = {NAME = 'clientantikick', DESC = 'Prevents localscripts from kicking you'}
...
function onDied()
	spawn(function()
		if pcall(function() game.Workspace.CloneCharacter:FindFirstChildOfClass('Humanoid') end) and game.Workspace.CloneCharacter:FindFirstChildOfClass('Humanoid') then
			game.Workspace.CloneCharacter:FindFirstChildOfClass('Humanoid').Died:connect(function()
				if game.Workspace.CloneCharacter:FindFirstChild("HumanoidRootPart") then
					lastDeath = game.Workspace.CloneCharacter:FindFirstChild("HumanoidRootPart").CFrame
				end
			end)
		else
			wait(2)
...
game.Workspace.CloneCharacterAdded:Connect(function()
	FLYING = false
	Floating = false
	...
	repeat wait() until game.Workspace.CloneCharacter:FindFirstChild('HumanoidRootPart')

	if spawnpoint and not refreshCmd and spawnpos ~= nil then
		wait(.1)
		game.Workspace.CloneCharacter.HumanoidRootPart.CFrame = spawnpos
	end
            ...
            if plr.Character and plr.Character:FindFirstChild('Head') then
				...
				local function espLoop()
					if PARENT:FindFirstChild(plr.Name..'_ESP') then
						if plr.Character and plr.Character:FindFirstChild('HumanoidRootPart') and plr.Character:FindFirstChild('Humanoid') and game.Workspace.CloneCharacter and game.Workspace.CloneCharacter:FindFirstChild('HumanoidRootPart') and game.Workspace.CloneCharacter:FindFirstChild('Humanoid') then
							local pos = math.floor((game.Workspace.CloneCharacter.HumanoidRootPart.Position - plr.Character.HumanoidRootPart.Position).magnitude)
...
IYMouse.Button1Down:connect(function()
	for i,v in pairs(binds) do
		if v.COMMAND == 'clicktp' then
			local input = v.KEY
			if input == 'RightClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) and game.Workspace.CloneCharacter then
				pcall(function() game.Workspace.CloneCharacter.HumanoidRootPart.CFrame = IYMouse.Hit + Vector3.new(0,7,0) end)
			elseif input == 'LeftClick' and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) and game.Workspace.CloneCharacter then
				pcall(function() game.Workspace.CloneCharacter.HumanoidRootPart.CFrame = IYMouse.Hit + Vector3.new(0,7,0) end)
			elseif UserInputService:IsKeyDown(Enum.KeyCode[input:sub(14)]) and game.Workspace.CloneCharacter then
				pcall(function() game.Workspace.CloneCharacter.HumanoidRootPart.CFrame = IYMouse.Hit + Vector3.new(0,7,0) end)
			end
...
vehicleflyspeed = 1
function sFLY(vfly)
	repeat wait() until Players.LocalPlayer and game.Workspace.CloneCharacter and game.Workspace.CloneCharacter:FindFirstChild('HumanoidRootPart') and game.Workspace.CloneCharacter:FindFirstChild('Humanoid')
	repeat wait() until IYMouse
	
	local T = game.Workspace.CloneCharacter.HumanoidRootPart
	...
	local function FLY()
		...
		spawn(function()
			repeat wait()
			if not vfly then
				game.Workspace.CloneCharacter:FindFirstChildOfClass('Humanoid').PlatformStand = true
			end
			...
			BG:destroy()
			BV:destroy()
			game.Workspace.CloneCharacter:FindFirstChildOfClass('Humanoid').PlatformStand = false
		end)
	end
	IYMouse.KeyDown:connect(function(KEY)
		if KEY:lower() == 'w' then
...
preMaxZoom = Players.LocalPlayer.CameraMaxZoomDistance
...
function noSitFunc()
	wait()
	if game.Workspace.CloneCharacter.Humanoid.Sit then
		game.Workspace.CloneCharacter.Humanoid.Sit = false
	end
end
addcmd('nosit',{},
	function(args, speaker)
	if noSit then noSit:Disconnect() nositDied:Disconnect() end
	noSit = game.Workspace.CloneCharacter.Humanoid:GetPropertyChangedSignal("Sit"):connect(noSitFunc)
	...
	if not r15(speaker) then
		...
		for i,v in pairs(players)do
			...
			bangLoop = game:GetService('RunService').Stepped:connect(function()
				pcall(function()
					game.Workspace.CloneCharacter.HumanoidRootPart.CFrame = Players[bangplr].Character.HumanoidRootPart.CFrame
				end)
			end)
		end
	else
...
addcmd('hatspin',{'spinhats'},
	...
	for _,v in pairs(speaker.Character.Humanoid:GetAccessories()) do
		...
		spinhats = game:GetService('RunService').Stepped:connect(function()
			pcall(function()
				keep.Position = game.Workspace.CloneCharacter.Head.Position
			end)
		end)
	end
end)
...
addcmd('autokeypress',{'keypress'},
	function(args, speaker)
		if keypress and keyrelease and args[1] then
			...
			local clickDelay = 0.1
			local releaseDelay = 0.1
			if args[2] and isNumber(args[2]) then clickDelay = tonumber(args[2]) end
			if args[3] and isNumber(args[3]) then releaseDelay = tonumber(args[3]) end
